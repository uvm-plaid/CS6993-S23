...
(* REVIEW whole section, preparing for inductive proof of isomorphism with nat *)
...
(* Can this be nested in bin_to_nat? *)
Fixpoint bn (m:bin) (r:nat) (d:nat) : nat :=
  match m with
  | Z => r
  | B0 m' => bn m' r (d+1)
  | B1 m' => bn m' (r + (exp 2 d)) (d+1)
  end.

(* I actually cannot define this as a Fixpoint (must use match?) *)
Definition bin_to_nat (m:bin) : nat :=
  bn m 0 0. 

Compute (bn (B1(B0(B1(B0(Z))))) 0 0).

Compute bin_to_nat (B1(B0(B1(B0(Z))))).

Example padding :
  bin_to_nat (B1(B1(B0(B1(B0(B0(Z))))))) = bin_to_nat (B1(B1(B0(B1(Z))))).
Proof.
  reflexivity. Qed.
